...
public static class Float64PlanarAngleUtils
{
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Float64PlanarAngle GetDirectedAngle(this IFloat64Vector2D pointA, IFloat64Vector2D pointB, IFloat64Vector2D pointC)
    {
        var v1 = pointA.Subtract(pointB);
        var v2 = pointC.Subtract(pointB);

        var angle1 = Math.Atan2(v1.Y.Value, v1.X.Value);
        var angle2 = Math.Atan2(v2.Y.Value, v2.X.Value);

        return (angle2 - angle1).RadiansToAngle().GetAngleInPositiveRange();
    }
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Float64PlanarAngle GetDirectedAngle(this IFloat64Vector2D v1, IFloat64Vector2D v2)
    {
        var angle1 = Math.Atan2(v1.Y.Value, v1.X.Value);
        var angle2 = Math.Atan2(v2.Y.Value, v2.X.Value);
